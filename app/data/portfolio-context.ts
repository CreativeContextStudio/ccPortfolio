...
function formatProjectsForContext(): string {
  return projects
    .map((project) => {
      const techStack = project.techStack.length > 0 ? project.techStack.join(', ') : 'N/A';
      const tags = project.tags.length > 0 ? ` | Tags: ${project.tags.join(', ')}` : '';

      return `${project.title} (${project.dateRange}) [${project.status}]
${project.description}
Pillar: ${project.pillar} | Tech: ${techStack}${tags}`;
    })
    .join('\n\n');
}
...
CONTACT & SOCIAL
Email: hiya@creativecontext.studio
LinkedIn: https://linkedin.com/in/jamesmckay
GitHub: https://github.com/jamesmckay
Resume: Available for download at /McKayResumeComplete2025.pdf

FOUR PILLARS OF EXPERTISE
Creative Context Studio's work is organized around four core pillars:
1. Creative Producing — Brand campaigns, documentaries, performance video, and immersive experiences for Fortune 500 clients and major networks.
2. Storytelling & Narrative — Narrative film, documentary, interactive experiences, and story-driven content across all formats.
3. Agentic Creative & AI — AI-native tools, generative workflows, interactive systems, and context-engineered experiences.
4. Line Producing & Production — Budget, schedule, crew, delivery. Network television, major studio features, and global distributed production.

COMPANY OVERVIEW
Creative Context Studio builds production systems that work at any scale, from boutique creative direction to distributed teams producing 100+ assets weekly. We combine two decades of traditional, brand, and broadcast experience (network television, Fortune 500 campaigns, film) with hands-on expertise in physical production with teams and crews all over the globe. Our expertise includes concepting narrative content, optimizing video, directing talent, solving technical problems, and building automated operations and content systems. We are equally comfortable with generative AI tools, context engineering, and agentic workflows. Creative Context Studio works remotely from anywhere on projects anywhere.
...
Frontend: React | Next.js | TypeScript | Tailwind CSS | Motion (Framer Motion) | HTML/CSS | Responsive Design
Backend: Node.js | Express | Python | PostgreSQL | REST APIs | API Design
DevOps & Infrastructure: Git | Vercel | Docker | CI/CD | AWS
Design: Figma | Design Systems | UI/UX | Prototyping
Creative Context Studio builds full-stack web applications including interactive portfolio sites, community storytelling platforms (storyATL), quick-reference tools (Bartender Friend), and AI-powered web experiences. Proficient in modern React ecosystem with server-side rendering, static generation, and API routes.

ADDITIONAL CAPABILITIES
Creative Strategy & Pitch Decks | Post-Production & VFX Supervision | Experiential & Immersive Installations | Brand Identity & Visual Systems | Cross-Platform Content Pipelines | Talent & Crew Management at Scale | Emerging Tech R&D & Prototyping

PRODUCTION MANAGEMENT SPECIALTIES
Budget Management | Production Scheduling | Tax Incentive Programs (Louisiana, international) | Risk Assessment | Multi-Location Coordination | Background Casting | Permit & Location Management
...
Agency Production Services (RF Studio53) | 2022 - 2025
...
Creative Context Studio has managed production logistics and creative delivery across diverse projects including Mariachi Gringo (US/Mexico co-production with tax credit management), Higher Ground (period film with complex stunts), We are the Hartmans (controlled practical effects), Were the World Mine (indie musical), Peter and Vandy (NYC location management), Death in Love (independent feature), and Behind the Star (10-episode web series). Creative Context Studio executed 10-episode web series for Crackle/Sony Pictures Television, directed JP Morgan Chase whiteboard animation campaign, and produced Conde Nast culinary video content.
...
AI / LLM / API SKILLS & EXPERTISE
Creative Context Studio builds production-grade AI systems and LLM-powered tools that translate ambiguous inputs into structured, usable outputs. We work across the full stack of modern AI development: Anthropic (Claude), OpenAI, and Google Gemini integrations; agentic workflows; tool/function calling; structured generation (JSON schema + validation); and human-in-the-loop (HITL) review experiences that keep creative control in the hands of the user.

On the LLM side, we specialize in contextual engineering: designing reliable context pipelines with retrieval-augmented generation (RAG), prompt layering, style constraints, and output contracts so results are consistent, brand-aligned, and easy to parse downstream. We build multi-step chains (plan, draft, critique, rewrite, finalize), routing and gating logic, and agentic orchestration patterns (planner/critic/editor, tool-using agents, supervisor + worker agents) for real workflows. We work with system prompts, prompt templates, few-shot examples, guardrails, response evaluation, and prompt/version management as part of an iterative shipping loop.

On the engineering side, we integrate AI via APIs end-to-end: OAuth, key management, streaming responses, retries/backoff, rate limiting, and telemetry. We design services that log full prompt lineage (inputs, model outputs, user edits, regenerated versions) and support evaluation and observability (quality scoring, regression checks, feedback capture, and audit trails). We have built patterns for agent memory, session state, tool invocation, and structured outputs that plug cleanly into product UIs and downstream systems.

CREATIVE PRODUCING & DIRECTION
Creative Context Studio is led by a creative producer and director with deep, hands-on experience taking ideas from concept to finished work that lands--on time, on brand, and at a level clients and audiences can feel. We deliver across documentary, broadcast, and branded/narrative formats, managing the full production lifecycle: creative development, packaging, budgeting, scheduling, casting/talent, crews, locations, legal/clearances, post pipelines, and final delivery. We operate in high-trust environments with demanding stakeholders, where the job is equal parts taste, logistics, and calm decision-making under pressure.

Our edge is building teams that make great things--fast, collaboratively, and without drama. We lead distributed crews and vendor networks across multiple time zones, aligning everyone around a clear creative north star and a realistic plan. We know how to staff lean when needed, scale up when the work demands it, and keep communication clean: strong briefs, tight pre-pro, clear ownership, rapid feedback loops, and schedule discipline that protects quality instead of squeezing it.

EDUCATION & ARTISTIC BACKGROUND
Theatrical Production & Performance | 1996-2005
About Face Theatre & PushPush Theater | Chicago, IL
Artistic Associate & Performer contributing to theatrical production and artistic development across multiple theatrical productions. Foundation in live performance, narrative storytelling, and creative collaboration that informs all of Creative Context Studio's work today.
